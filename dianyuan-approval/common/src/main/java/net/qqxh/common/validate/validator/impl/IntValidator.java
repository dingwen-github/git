 ...
 * 〈数字校验〉<br>
 * 〈例如：@Validate(rules = "count=int:{max:1,min:20,msg:'请输入1-20之间的数字'};") max为最大值 min为最小值msg为校验失败提醒〉
 *
 * @author jason
...
public class IntValidator implements Validator {
    ...
    public boolean match(String val, JSONObject validateRule) {
        ...
        try {
            int min = validateRule.getIntValue("min");
            int max = validateRule.getIntValue("max");
